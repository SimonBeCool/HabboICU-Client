...
import { LOGIN_OK, MAP_DATA, ROOM_ITEM_DATA, PLAYERS_DATA, PLAYER_STATUS, PLAYER_REMOVE, INVENTORY_ITEMS, CHAT, PLAYER_WAVE, ITEM_REMOVE, ITEM_STATE, WALL_ITEM_DATA, INVENTORY_ITEM_REMOVE } from "./protocol/OpCodes/ServerOpCodes";
...
import HandleInventoryItems from "./incoming/rooms/HandleInventoryItems";
import HandleInventoryItemRemove from "./incoming/rooms/HandleInventoryItemRemove";
...
export default class CommunicationManager implements IMessageHandler {
    ...
    handleOpenConnection = (): void => {
        BobbaEnvironment.getGame().uiManager.log("Connected with Bobba-Server v0.0.8!!");
    }
    ...
    connect(host: string, port: number, secure: boolean): Promise<void> {
        const connectionURL = (secure ? 'ws' : 'ws') + '://' + host + ':' + port;
