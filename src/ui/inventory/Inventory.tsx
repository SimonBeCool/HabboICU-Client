            ...
            .forEach(item => {
                ...
                if ((item.stackable && stackableItemsDrawn[item.baseId]++ === 0) || !item.stackable) {
                    if (item.baseItem != null) {
                        const image = "hof_furni/"+item.baseItem.furniBase.itemData.classname+"/"+item.baseItem.furniBase.itemData.classname+"_icon_a.png";
                        displayItems.push(
                            <button key={item.id} onClick={this.handleSelectItem(item.id)} className={item.id === selectedId ? 'selected' : ''}>
                                <img src={image} alt={item.baseItem.furniBase.itemData.name} />
